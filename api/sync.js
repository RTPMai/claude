...
export default async function handler(req, res) {
  ...
  try {
    if(mode==="invoices") {
      // Fetch ALL invoices (for outstanding balance) — paginated with resume
      let existing=[];
      ...
      if(cursor){
        ...
        return res.status(200).json({ok:true,status:"invoices_done",fetched:nodes.length,pages,nextUrl:"/api/sync?mode=invoices-ytd"});
      }
    }

    else if(mode==="invoices-ytd") {
      // Fetch ONLY this year's invoices using inProductionAfter for accurate YTD sales
      const year=new Date().getFullYear();
      let existing=[];
      try{ const p=await kvGet("printavo_invoices_ytd_partial"); if(p&&p.nodes) existing=p.nodes; }catch(e){}
      if(!resumeCursor) existing=[];
      ...
      do {
        const after=cursor?`,after:"${cursor}"`:""
        const data=await gql(`query{invoices(first:25,sortOn:VISUAL_ID${yearFilter}${after}){nodes{id visualId createdAt total amountOutstanding status{id name}contact{fullName}}pageInfo{hasNextPage endCursor}}}`);
        nodes.push(...data.invoices.nodes);
        cursor=data.invoices.pageInfo.hasNextPage?data.invoices.pageInfo.endCursor:null;
        pages++;
        if(cursor) await new Promise(r=>setTimeout(r,1200));
      ...
      await kvSet("printavo_invoices_ytd_partial",{nodes,cursor,updatedAt:new Date().toISOString()});

      if(cursor){
        return res.status(200).json({ok:true,status:"partial",fetched:nodes.length,pages,nextCursor:cursor,nextUrl:`/api/sync?mode=invoices-ytd&cursor=${encodeURIComponent(cursor)}`});
      } else {
        return res.status(200).json({ok:true,status:"invoices_ytd_done",fetched:nodes.length,pages,nextUrl:"/api/sync?mode=quotes"});
      }
    }
      ...
      const invYtdPartial = await kvGet("printavo_invoices_ytd_partial");
      const quoPartial    = await kvGet("printavo_quotes_partial");
      const statuses      = await fetchAllStatuses();

      const invoices    = invPartial&&invPartial.nodes    ? invPartial.nodes    : [];
      const invoicesYtd = invYtdPartial&&invYtdPartial.nodes ? invYtdPartial.nodes : [];
      const quotes      = quoPartial&&quoPartial.nodes   ? quoPartial.nodes    : [];

      await kvSet("printavo_data",{ invoices, invoicesYtd, quotes, statuses, syncedAt:new Date().toISOString() });

      return res.status(200).json({ok:true,status:"complete",invoices:invoices.length,invoicesYtd:invoicesYtd.length,quotes:quotes.length,statuses:statuses.length,syncedAt:new Date().toISOString()});
    }

    else {
      return res.status(400).json({ error:"Invalid mode. Use: invoices, invoices-ytd, quotes, finalize" });
    }
